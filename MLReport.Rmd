...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;+ Boosting (see http://topepo.github.io/caret/training.html)    
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;+ C5.0 Decision Trees    
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;+ Naive Bayes    
...
With selected Random Forest model, accuracy is 99.69% with out-of-bag error rate 0.31%. 
...
Preliminary analysis of the data showed 2 issues    
* some columns has big amount of 'NA's     
* strange '#DIV/0!' strings on data     
...
* "raw_timestamp_part_1","raw_timestamp_part_2","cvtd_timestamp",                                                      "new_window","num_window": they looks like to be training date & time related data, not needed for prediction as it should be assumed that the person will train at any point of time not just in certain date and time     
...
After cleaning dataset contains `r nrow(cleanedData)` observations with `r ncol(cleanedData)` variables. We have removed `r ncol(trainRawData)-ncol(cleanedData)` variables from the test data. See appendix 1 for variable importance.
...
I will use tit model with boosting algorithm and 10-fold cross validation to predict classe with all other predictors.    
...
### C5.0 Decision Trees    
While preanalyzing the data, it can be seen that the CSV files are ordered. For C5.0 to  work, data needs to be in random order so boost-632 will be used.
...
* out-of-bag error rate `r round(verificationAccuracy$OutOfSampleError[4],2)`    
...
Most probably the amount of variables could be dropped more. The computation time with 6 threads was good enough, so all 52 varibles was kept when defined models were created.
